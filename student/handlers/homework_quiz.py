...
async def handle_test_back_navigation(callback: CallbackQuery, state: FSMContext):
    ...
    await cleanup_test_data(callback.from_user.id)
    logging.info(f"🧹 Очищены данные теста при возврате для пользователя {callback.from_user.id}")
...
async def finish_test(chat_id, state: FSMContext, bot):
    ...
    logging.info(f"🏁 ФУНКЦИЯ: finish_test | СОСТОЯНИЕ ПЕРЕД ОЧИСТКОЙ: {current_state_before}")

    # НЕ очищаем состояние полностью, чтобы сохранить роль пользователя
    # Вместо этого просто сбрасываем состояние на None, но данные остаются
    await state.set_state(None)

    # Логируем состояние после сброса
    current_state_after = await state.get_state()
    logging.info(f"🏁 ФУНКЦИЯ: finish_test | СОСТОЯНИЕ ПОСЛЕ СБРОСА: {current_state_after}")

    # Очищаем данные о завершенном тесте
    await cleanup_test_data(data.get("user_id"))
